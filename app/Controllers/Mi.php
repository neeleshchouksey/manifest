...
class Mi extends BaseController
{
    ...
    public function analysis_and_planning()
    {
        $db = \Config\Database::connect();
        $commonModel = new CommonModel($db);
        $data['all_project_data'] = $commonModel->getRecords('portfolio_project', '*', array('is_active' => 1, 'del_status' => 0));
    ...
    public function design_and_html()
    {
        $db = \Config\Database::connect();
        $commonModel = new CommonModel($db);
        $data['all_project_data'] = $commonModel->getRecords('portfolio_project', '*', array('is_active' => 1, 'del_status' => 0));
    ...
    public function development_and_implimentation()
    {
        $db = \Config\Database::connect();
        $commonModel = new CommonModel($db);
        $data['all_project_data'] = $commonModel->getRecords('portfolio_project', '*', array('is_active' => 1, 'del_status' => 0));
    ...
    public function laravel_developer()
    {
        $db = \Config\Database::connect();
        $commonModel = new CommonModel($db);
        ...
        $data['description'] = "We are hiring Laravel developer with more than 2 years of experience in programming";
        $data['keywords'] = "";
        ...
        echo view('includes/header', $data);
        echo view('laravel-developer');
        echo view('portfolio/related_projects');
        echo view('includes/footer');
    }
    
    public function hiring_react_js_developer()
    {
        $db = \Config\Database::connect();
        $commonModel = new CommonModel($db);
        $miModel = new MiModel($db);
        ...
        echo view('includes/header', $data);
        echo view('hiring-react-js-developer');
        echo view('portfolio/related_projects');
        echo view('includes/footer');
    }

    public function business_development_officer()
    {
        $db = \Config\Database::connect();
        $commonModel = new CommonModel($db);
        $miModel = new MiModel($db);
    ...
    function emailsender()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$input) {
                ...
                $email->setFrom($this->request->getPost('email'), $this->request->getPost('fname'));
                $email->setTo("nchouksey@manifestinfotech.com");
                $email->setReplyTo($this->request->getPost('email'), $this->request->getPost('fname'));
                $email->setSubject($this->request->getPost('subject'));
                $msg = $this->request->getPost('msg') ;
                $phone = $this->request->getPost('phone');
                if ($this->request->getPost('phone')) {
                    $message_contact = $this->request->getPost('msg') . "<br><br>" . "Email : " . $this->request->getPost('email') . "<br>" . "Contact no. : " . $phone;
                } else {
                    $message_contact = $this->request->getPost('msg') . "<br><br>" . "Email : " . $this->request->getPost('email');
                }

                $email->setMessage($message_contact);
                // $email->send();
                ...
                // send email
                if ($email->send()) {
                    echo 'Thanks for your message, We will reply back soon!';
                } else {
                    $data = $email->printDebugger(['headers']);
                    print_r($data);
                }
